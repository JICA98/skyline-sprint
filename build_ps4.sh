...
if ! grep -q 'SkylineSprint-0.1.5-ui-polish' src/platform/platform.cpp; then
...
echo "Source marker: SkylineSprint-0.1.5-ui-polish"
...
if command -v strings >/dev/null 2>&1; then
    if ! strings package/eboot.bin | grep 'SkylineSprint-0.1.5-ui-polish' >/dev/null; then
